...
We could omit the initialize method because it would be inherited from the Pets class. I prefer this way, as I coded above, because the information set in Pets is not specific to cats and could be used for other Pets subclasses. The string message is specific to cats and so I decided to place it in the Cats subclass. Prob;lems might arise with this approach when trying to reassign Instance Variables in a subclass, one would need to go into the parent class to do so.  
=end